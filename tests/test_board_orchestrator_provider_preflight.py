...
class FakeKanboardReview:
    def __init__(self, *, repo_path: str):
        ...
        self.tasks = {
            self.review_id: {
                "id": self.review_id,
                "title": "Review task should not spawn when provider is blocked",
                "description": f"Repo: {repo_path}\n",
        ...
        self.tags_by_task_id: dict[int, list[str]] = {self.review_id: []}
            ...
            return [
                {
                    ...
                    "columns": [
                        ...
                        {"id": self.col_wip, "title": "Work in progress", "tasks": []},
                        {"id": self.col_review, "title": "Review", "tasks": [self._card(self.review_id)]},
                        {"id": self.col_blocked, "title": "Blocked", "tasks": []},
                        {"id": self.col_done, "title": "Done", "tasks": []},
                    ],
                }
...
class TestProviderPreflight(unittest.TestCase):
    ...
    def test_reviewer_auth_failure_blocks_spawn_and_tags_task(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp) / "repo"
            repo.mkdir()

            fake = FakeKanboardReview(repo_path=str(repo))
            ...
            old_preflight = bo.preflight_claude
            ...
            try:
                ...
                bo.STATE_PATH = str(state_path)
                bo.LOCK_PATH = str(Path(tmp) / "lock.json")
                # Heuristic should infer claude based on "spawn_reviewer" substring.
                bo.REVIEWER_SPAWN_CMD = "/Users/joshwegener/clawd/scripts/spawn_reviewer_tmux.sh {task_id} {repo_key} {repo_path} {patch_path} {review_revision}"
                bo.PREFLIGHT_ENABLED = True

                def _preflight_claude(*, timeout_sec: int):
                    calls["preflight"] += 1
                    return {"ok": False, "category": "auth", "message": "Not logged in"}

                def _spawn_reviewer(*_a, **_k):
                    calls["spawn"] += 1
                    return {"execSessionId": "pid:999", "logPath": str(Path(tmp) / "r.log")}

                bo.preflight_claude = _preflight_claude  # type: ignore[assignment]
                bo.spawn_reviewer = _spawn_reviewer  # type: ignore[assignment]
                ...
                payload1 = json.loads(buf1.getvalue().strip())
                self.assertTrue(any("provider claude unavailable" in e for e in payload1.get("errors", [])))

                tags = {t.lower() for t in fake.tags_by_task_id[fake.review_id]}
                self.assertIn("blocked:auth", tags)
                self.assertIn("auto-blocked", tags)
                # Should not have moved to a different column.
                self.assertEqual(fake.tasks[fake.review_id]["column_id"], fake.col_review)
                # Spawn should not have been attempted.
                self.assertEqual(calls["spawn"], 0)
                self.assertEqual(calls["preflight"], 1)
                ...
                bo.preflight_claude = old_preflight
